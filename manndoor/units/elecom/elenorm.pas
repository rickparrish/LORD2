...
begin
  {$IFDEF WIN32}
    Case ComSystem of
      ...
      3 : PWC5Obj(ComObj)^.Com_ReadProc(TempPtr);
    end; { case }
  {$ENDIF}
...
begin
  {$IFDEF WIN32}
    Case ComSystem of
      ...
      3 : PWC5Obj(ComObj)^.Com_WriteProc(TempPtr);
    end; { case }
  {$ENDIF}
...
begin
  ...
  Case Objecttype of
    ...
    {$IFDEF UNIX}
      01 : ComObj := New(pUnixObj, Init);
      02 : ComObj := New(pUnixObj, Init);
    {$ENDIF}
  end; { case }
...
begin
  Case ComSystem of
    {$IFDEF WIN32}
      01 : Dispose(pWin32Obj(ComObj), Done);
      02 : Dispose(pTelnetObj(ComObj), Done);
      03 : Dispose(pWC5Obj(ComObj), Done);
    {$ENDIF}

    {$IFDEF OS2}
      01 : Dispose(pOs2Obj(ComObj), Done);
      02 : Dispose(pTelnetObj(ComObj), Done);
    {$ENDIF}

    {$IFDEF MSDOS}
      01 : Dispose(pFossilObj(ComObj), Done);
    {$ENDIF}
    ...
    {$IFDEF UNIX}
      01 : Dispose(pUnixObj(ComObj), Done);
      02 : Dispose(pUnixObj(ComObj), Done);
    {$ENDIF}
  end; { case }
