...
{ The StrToHash function works the same as MD5String
  but it returns a string value instead of TMD5Digest }
function StrToHash(const S: String): String;
...
var
  Digest: TMD5Digest;
begin
     Digest := MD5Buffer(PChar(S)^, Length(S));
     Result := IntToHex(Digest.A, 8) + IntToHex(Digest.B, 8) + IntToHex(Digest.C, 8) + IntToHex(Digest.D, 8);
end;
